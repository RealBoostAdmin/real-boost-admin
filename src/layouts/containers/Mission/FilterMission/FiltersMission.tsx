...
import {MissionFilterModel} from '../../../../core/models/mission/mission.model';
import TextField from '@mui/material/TextField';

interface IPropsFiltersMission {
    filters?: MissionFilterModel;
    filtersEnabled: FilterEnabledModel[];
    setFiltersEnabled: (filters: FilterEnabledModel[]) => void;
}

const FiltersMission = ({
                            filters,
                            filtersEnabled,
                            setFiltersEnabled,
                        }: IPropsFiltersMission) => {
    const styles = {
        display: 'flex',
        flexDirection: 'row',
        margin: '1em 0',
    ...
    return (
        <div style={styles}>
            {filters && (
                <>
                    <TextField
                        onChange={(e) => changeValue('code', e.currentTarget.value, FilterType.Unique)}
                        type="text"
                        label="Code"
                        InputLabelProps={{shrink: true}}
                    />
                    <TextField
                        onChange={(e) => changeValue('user->>username', e.currentTarget.value, FilterType.Unique)}
                        type="text"
                        label="Username"
                        InputLabelProps={{shrink: true}}
                    />
